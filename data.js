...
const DB = {
    ...
    projects: [
      {
        id: 1,
        name: 'NaijaRadar',
        tag: 'UI/UX Design · Flight Tracking',
  ...
  get(key) {
    try {
      ...
      if (!raw) return this.cloneDefault(key);
      return JSON.parse(raw);
    } catch (e) {
      return this.cloneDefault(key);
    }
  },
  ...
  set(key, value) {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('pf-data-updated', {
          detail: { key, value }
        })
      );
  ...
  init() {
    Object.keys(this.defaults).forEach((key) => {
      if (localStorage.getItem(this.KEYS[key]) === null) {
